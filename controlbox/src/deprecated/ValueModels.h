...
class ExternalReadOnlyValue : public ReadableObject
{
protected:
	void* const _pValue;
	uint8_t const _size;
...
class ExternalValue : public ExternalReadOnlyValue, public WritableObject
{
public:
...
template <class T> class TransientValue : public WritableObject
{
	T value;
public:
...
private:
	...
	void fetchTarget() {
		...
		previous = (ReadableObject*)lookupUserObject(root(), data);
	}
public:
	...
	virtual uint8_t writeStreamSize() override { return ((WritableObject*)previous)->writeStreamSize(); }
	virtual void writeFrom(DataIn& dataIn) override { if (previous) ((WritableObject*)previous)->writeFrom(dataIn); }
