...
SCENARIO("Basic test of chain of filters", "[filterchain]")
{
    std::vector<std::vector<uint8_t>> chainsSpecs = {{0},
                                                     {0, 1},
                                                     {0, 1, 1}};

    for (auto chainSpec = chainsSpecs.begin(); chainSpec != chainsSpecs.end(); chainSpec++) {
        std::stringstream chain_str;
        std::ostream_iterator<int> out_it(chain_str, ", ");
