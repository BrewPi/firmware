...
char * toStringImpl(int32_t const raw, // raw value of fixed point
...
public:
    ...
    bool fromString(char const * const s, int32_t minimum = min_val, int32_t maximum =
            max_val) {
        int32_t tempValue;
    ...
    bool fromTempString(char const * const s, char format, bool absolute, int32_t minimum = min_val, int32_t maximum =
                max_val) {
        int32_t tempValue;
        bool success = fromStringImpl(&tempValue, fractional_bit_count, s,
